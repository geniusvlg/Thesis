...
class MuabannhadatSpider(scrapy.Spider):
	...
	def start_requests(self):
		...
		urls = [
		'https://dothi.net/nha-dat-ban.htm',
		'https://dothi.net/nha-dat-cho-thue.htm'
		]
		for url in urls:
	...
	def parse(self, response):  
		...
		# Go to the next page
		next_page = response.xpath("//div[contains(@class,'-next-first-last')]/parent::*/@href").extract_first()
		if next_page.find("p") != -1:
